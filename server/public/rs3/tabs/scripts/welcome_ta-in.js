	    ...
	    a.exports.tabPage = function (b) {
			...
			g = function () {
			    ...
			    function btnFontResize(scale) {
			        ...
			        var getStartFontSize = Number(btnFontSize) + 1;
			        if (browserWidth < 700)
			            getStartFontSize = Number(btnFontSize) + 1;
			        $('#links span').css('font-size', getStartFontSize + 'px');
			    }
			    function headerTextResize(scale) {
			        if (scale != 'inherit') {
			            scale *= d;
			        }
			        $('#welcome-tagline1').css('font-size', scale);
			        $('#welcome-tagline2').css('font-size', scale);
			        $('#welcome-tagline3').css('font-size', scale);
			    }

			    // Button font resize 
			    if ($(window).width() < 1200 && $(window).width() > 910) {
			        btnFontResize(100);
			        headerTextResize(1.9);
			    }
			    else if ($(window).width() <= 910 && $(window).width() > 750) {
			        btnFontResize(95);
			        headerTextResize(1.8);
			    }
			    else if ($(window).width() <= 750 && $(window).width() > 650) {
			        btnFontResize(95);
			        headerTextResize(1.5);
			    }
			    else if ($(window).width() <= 650) {
			        btnFontResize(95);
			        headerTextResize(1.4);
			    }
			    else {
			        $('#links .links-button').css('font-size', '13px');
			        $('#links span').css('font-size', '19px');

			        headerTextResize('inherit');
			    }
